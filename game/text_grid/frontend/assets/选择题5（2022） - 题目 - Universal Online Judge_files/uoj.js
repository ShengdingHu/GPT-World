...
$.fn.uoj_blog_tag = function() {
	return this.each(function() {
		$(this).attr('href', blogHome + '/archive?tag=' + encodeURIComponent($(this).text()));
	});
}
...
function showStandings() {
	$("#standings").long_table(
		...
		function(row) {
			...
			col_tr += '<td>' + '<div><span class="uoj-score" data-max="' + problems.length * 100 + '" style="color:' + getColOfScore((row[0]+row[3]) / problems.length) + '">' + Math.round(row[0]+row[3]) + '</span></div>' + '<div>' + getPenaltyTimeStr(row[1]) + '</div></td>';
			for (var i = 0; i < problems.length; i++) {
				col_tr += '<td>';
				col = score[row[2][0]][i];
				if (col != undefined) {

					col_tr += '<div><a href="/submission/' + col[2] + '">';
					col_tr += '<span class="uoj-score" style="color:' + getColOfScore(col[0]) + '">' + col[0] + '</span>';
					if(style_proportion && style_proportion[i]){
						col_tr += '<sub>*' + (1 - style_proportion[i]).toFixed(2) + '</sub>';
						col_tr += '&nbsp;&nbsp;+&nbsp;&nbsp;'
						if(col[3]){
							col_tr += '<span class="uoj-score" style="color:' + getColOfScore(col[3]) + '">' + col[3] + '</span>';
						}else{
							col_tr += '<span class="uoj-score" style="color:rgb(150,150,150)">??</span>'
						}
						col_tr += '<sub>*' + style_proportion[i].toFixed(2) + '</sub>';
					}
					col_tr += '</a></div>';
